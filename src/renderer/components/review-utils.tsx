...
export function getFocusCorrection(preview: ReviewPreviewSnapshot): AnchoredCorrectionOperationSnapshot | null {
  ...
  const matches = preview.operations.corrections.filter(
    (correction) => correction.correctionIndex === focusIndex && correction.status !== 'low_confidence',
  );
...
export function HighlightedWriting({
  content,
  corrections,
}: {
  content: string;
  corrections: AnchoredCorrectionOperationSnapshot[];
}): React.JSX.Element {
  const anchoredCorrections = corrections
    .filter(
      (correction) =>
        correction.status !== 'low_confidence' && correction.startOffset !== null && correction.endOffset !== null,
    )
  ...
  anchoredCorrections.forEach((correction) => {
    ...
    parts.push(
      <mark className="review-highlight" key={`${startOffset}-${endOffset}`}>
  ...
  return (
    <div
      className="max-h-36 overflow-y-auto whitespace-pre-wrap rounded-2xl border border-warning/30 bg-warning/10 p-4 text-sm leading-7 text-base-content/80 scrollable"
...
export function CorrectionCard({ correction, showAnswer, reason }: CorrectionCardProps): React.JSX.Element {
  return (
    <article
      className={`rounded-2xl border p-4 ${correction.status === 'low_confidence' ? 'border-base-300 border-dashed bg-base-200/70' : 'border-base-300 bg-base-100'}`}
    >
      <div className="mb-3 flex items-center justify-between gap-3">
        <span className="badge badge-soft badge-primary">
          {correction.status === 'low_confidence' ? 'Suggestion' : 'Fix'}
        </span>
        <span className="text-xs font-medium uppercase tracking-[0.16em] text-base-content/40">
          {correction.category}
        </span>
      </div>
      <div className="space-y-3 text-sm leading-6">
        <ReviewField
          label="Pattern"
          value={correction.matchedPatternId ?? patternRule(correction) ?? correction.category}
        />
        <ReviewField label="You wrote" value={correction.originalText} />
        <ReviewField
          label="Try"
          value={showAnswer ? correction.correctedText : 'Hidden until you try or reveal.'}
          strong={showAnswer}
        />
